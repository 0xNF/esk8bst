...
const forSale: Array<RegExp> = [
    new RegExp("[\\[\\(].*?(sell|sale|wts|lts).*?[\\)\\]]", 'i'),
    new RegExp("(WTS|SELL|LTS)[:,]?"),
];
const forPurchase: Array<RegExp> = [
    new RegExp("[\\[\\(].*?(buy|ltb|wtb|purchase).*?[\\)\\]]", 'i'),
    new RegExp("(WTB|BUY|LTB)[:,]?"),
]
const forTrade: Array<RegExp> = [
    new RegExp("[\\[\\(].*?(trade|wtt|swap).*?[\\)\\]]", 'i'),
    new RegExp("(TRADE|WTT|LTT)[:,]?"),
]
function _parseBuySellTradeType(s: string): BuySellTrade {
    ...
    for (let i = 0; i < forPurchase.length; i++) {
        const rgx = forPurchase[i];
        if (rgx.test(s)) {
            return "BUY";
        }
    }
    for (let i = 0; i < forTrade.length; i++) {
        const rgx = forTrade[i];
        if (rgx.test(s)) {
            return "TRADE";
        }
    }
