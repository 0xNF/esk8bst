...
interface FilterZoneProps {
  ...
  OnNotifyOpen: () => void;
  LoadedThreads: Array<IBSTThread>;
}
  ...
  return (
    <div className="FilterZone">
      ...
      <div>
        ...
        {
          (props.Threads && (props.Threads.reduce((prev, curr) => { return prev + curr.BSTs.length }, 0) !== props.LoadedThreads.reduce((prev, curr) => { return prev + curr.BSTs.length }, 0))) ?
            <div>
              <span>Results: {props.Threads.reduce((prev, curr) => { return prev + curr.BSTs.length }, 0)} / {props.LoadedThreads.reduce((prev, curr) => { return prev + curr.BSTs.length }, 0)} </span>
            </div>
            :
            ""
        }
